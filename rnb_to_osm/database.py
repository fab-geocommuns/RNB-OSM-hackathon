...
class Export(db.Model):
    ...
    def export_file_name(self) -> str:
        ...
        if (
            ...
            return f"rnb_{self.id}_{self.export_params['code_insee']}_{formatted_created_at}.osm"
        elif "bbox" in self.export_params and self.export_params["bbox"] is not None:
            return f"rnb_{self.id}_{formatted_created_at}.osm"
        else:
...
def import_rnb_buildings(db: SQLAlchemy) -> None:
    ...
    db.session.execute(
        text(
            ...
            ALTER TABLE rnb_buildings_temp DROP COLUMN point;
            ALTER TABLE rnb_buildings_temp DROP COLUMN status;
